...
elseif MODEL == 5
    ...
    for i = 2:T
        ...
        % VaR(i) = BETA(1) + BETA(2) * VaR(i-1) + BETA(3) * y(i-1)^3 + BETA(4) * y(i-1)^4;
        derivative1(i) = 1 + BETA(2)*derivative1(i-1);
        derivative2(i) = VaR(i-1) + BETA(2)*derivative2(i-1);
        derivative3(i) = BETA(2)*derivative3(i-1) + y(i-1)^3;
        derivative4(i) = BETA(2)*derivative4(i-1) + y(i-1)^4;
...
elseif MODEL == 6
    ...
    for i = 2:T
        ...
        % VaR(i) = BETA(1) + BETA(2) * VaR(i-1) + BETA(3) * y(i-1)^3 * I(y>0) + BETA(4) * y(i-1)^3 * I(y<=0) +
        %            BETA(5) * y(i-1)^4 * I(y>0) +  BETA(6) * y(i-1)^4 * I(y<=0) ;
        derivative1(i) = 1 + BETA(2)*derivative1(i-1);
        derivative2(i) = VaR(i-1) + BETA(2)*derivative2(i-1);
        derivative3(i) = BETA(2)*derivative3(i-1) + y(i-1)^3 * (y(i-1)>0);
        derivative4(i) = BETA(2)*derivative4(i-1) + y(i-1)^3 * (y(i-1)<=0);
        derivative5(i) = BETA(2)*derivative5(i-1) + y(i-1)^4 * (y(i-1)>0);
        derivative6(i) = BETA(2)*derivative6(i-1) + y(i-1)^4 * (y(i-1)<=0);
        
        gradient(i,:) = [derivative1(i), derivative2(i), derivative3(i), derivative4(i), derivative5(i), derivative6(i)];   
